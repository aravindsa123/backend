...
app.post('/new',upload.single('image1'),async (request,response) => {
    try {
        const { name, email, phone, hospital, specialization, experience, qualification,location, gender} = request.body
        const newdata = new catemodel({
            name,email,phone,hospital,specialization,experience,qualification,location,gender,
            image1: {
...
app.post('/UReg',async (request,response) => {
    try {
        ...
        const newdata = new data4model({
            fullname,email,phone,password,age
        })
        console.log(newdata);
        await newdata.save();
